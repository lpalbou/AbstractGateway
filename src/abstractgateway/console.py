...
def _core_console_parts() -> Dict[str, Any]:
    ...
    try:
        ...
        required = (support.get("required") if isinstance(support, dict) else None) or "2.16.0"
...
<body>
  ...
  <script>
    ...
    function coreConsoleUnavailableText() {
      if (!CORE_CONSOLE.available) return `${CORE_CONSOLE.message || "Models and Engines require abstractcore ≥ 2.16.0."} Upgrade: ${CORE_CONSOLE.upgrade || 'pip install -U "abstractcore>=2.16.0"'}`;
