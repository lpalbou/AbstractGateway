...
def test_gateway_attachments_ingest_creates_session_scoped_artifact(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runtime_dir = tmp_path / "runtime"
    bundles_dir = tmp_path / "bundles"
    _write_min_bundle(bundles_dir=bundles_dir, bundle_id="bundle-attachments", flow_id="root")
    ...
    with TestClient(app) as client:
        r = client.post(
            ...
            json={"session_id": "s1", "path": "notes.txt"},
            headers=headers,
        )
...
def test_gateway_attachments_ingest_accepts_at_prefixed_virtual_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runtime_dir = tmp_path / "runtime"
    bundles_dir = tmp_path / "bundles"
    _write_min_bundle(bundles_dir=bundles_dir, bundle_id="bundle-attachments-at", flow_id="root")
    ...
    with TestClient(app) as client:
        r = client.post(
            "/api/gateway/attachments/ingest",
            json={"session_id": "s3", "path": "@notes.txt"},
            headers=headers,
        )
        assert r.status_code == 200, r.text
        body = r.json()
