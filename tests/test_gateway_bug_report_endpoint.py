...
def test_gateway_bug_report_endpoint_creates_template_and_collision_safe_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    runtime_dir = tmp_path / "runtime"
    bundles_dir = tmp_path / "bundles"
    _write_min_bundle(bundles_dir=bundles_dir, bundle_id="bundle-bugs", flow_id="root")
    ...
    with TestClient(app) as client:
        r1 = client.post(
            "/api/gateway/bugs/report",
            headers=headers,
            json={"session_id": "s1", "description": description, "active_run_id": "r1", "workflow_id": "bundle-bugs:root"},
        )
        ...
        bug_dir = runtime_dir / "bug_reports"
        template_path = bug_dir / "template.md"
        assert template_path.exists()

        report1 = bug_dir / filename1
        ...
        report2 = bug_dir / filename2
        assert report2.exists()


def test_gateway_bug_report_endpoint_auto_bridges_to_typed_proposed_backlog_when_repo_available(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    runtime_dir = tmp_path / "runtime"
    bundles_dir = tmp_path / "bundles"
    _write_min_bundle(bundles_dir=bundles_dir, bundle_id="bundle-bugs-bridge", flow_id="root")
    ...
    with TestClient(app) as client:
        r = client.post(
            "/api/gateway/bugs/report",
            headers=headers,
            json={"session_id": "s1", "description": description, "active_run_id": "r1", "workflow_id": "bundle-bugs-bridge:root"},
        )
