...
def test_base_install_is_remote_light_server() -> None:
    ...
    assert "AbstractRuntime[multimodal]>=0.4.11" in deps
    assert "abstractagent>=0.3.7" in deps
    assert "abstractcore[remote,media,tools,tokens,compression,vision,voice,audio]>=2.13.14" in deps
    assert "abstractflow>=0.3.11" in deps
    assert "abstractvision>=0.3.5" in deps
    assert "abstractvoice>=0.9.4" in deps
...
def test_entrypoint_profiles_cascade_lower_package_extras() -> None:
    ...
    apple = "\n".join(extras["apple"])
    assert "AbstractRuntime[multimodal,all-apple]>=0.4.11" in apple
    assert "abstractagent[all-apple]>=0.3.7" in apple
    assert "abstractcore[all-apple]>=2.13.14" in apple
    assert "abstractvision[all-apple]>=0.3.5" in apple
    assert "abstractvoice[all-apple]>=0.9.4" in apple
    assert "abstractmusic[all-apple]>=0.1.1" in apple
    assert "AbstractMemory[all-apple]>=0.2.6" in apple
    gpu = "\n".join(extras["gpu"])
    assert "AbstractRuntime[multimodal,all-gpu]>=0.4.11" in gpu
    assert "abstractagent[all-gpu]>=0.3.7" in gpu
    assert "abstractcore[all-gpu]>=2.13.14" in gpu
    assert "abstractvision[all-gpu]>=0.3.5" in gpu
    assert "abstractvoice[all-gpu]>=0.9.4" in gpu
