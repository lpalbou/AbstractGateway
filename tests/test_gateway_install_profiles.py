...
def test_base_install_keeps_remote_light_multimodal_plugins_without_local_inferencers() -> None:
    ...
    assert "abstractcore[remote,tools,vision,voice,audio,music]>=2.15.3" in runtime_base
...
def test_entrypoint_profiles_cascade_lower_package_extras() -> None:
    ...
    assert "abstractcore[embeddings]>=2.15.3" in embeddings
    ...
    apple = "\n".join(extras["apple"])
    assert "AbstractRuntime[apple]>=0.4.36" in apple
    ...
    gpu = "\n".join(extras["gpu"])
    assert "AbstractRuntime[gpu]>=0.4.36" in gpu
...
def test_default_docker_image_uses_base_server_and_nvidia_uses_gpu_profile() -> None:
    ...
    assert "ghcr.io/lpalbou/abstractgateway:${ABSTRACTGATEWAY_IMAGE_TAG:-0.4.4}" in compose
    ...
    assert "ghcr.io/lpalbou/abstractgateway:${ABSTRACTGATEWAY_NVIDIA_IMAGE_TAG:-0.4.4-gpu}" in nvidia_compose
