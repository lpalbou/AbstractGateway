...
def test_base_install_is_remote_light_server() -> None:
    ...
    assert "abstractcore>=2.16.0" in deps
    assert "abstractagent>=0.3.14" in deps
...
def test_entrypoint_profiles_cascade_lower_package_extras() -> None:
    ...
    assert "abstractcore[embeddings]>=2.16.0" in embeddings
    ...
    apple = "\n".join(extras["apple"])
    assert "AbstractRuntime[apple]>=0.5.0" in apple
    assert "abstractagent[apple]>=0.3.14" in apple
    ...
    gpu = "\n".join(extras["gpu"])
    assert "AbstractRuntime[gpu]>=0.5.0" in gpu
    assert "abstractagent[gpu]>=0.3.14" in gpu
...
def test_default_docker_image_uses_base_server_and_nvidia_uses_gpu_profile() -> None:
    ...
    assert "ghcr.io/lpalbou/abstractgateway:${ABSTRACTGATEWAY_IMAGE_TAG:-0.5.0}" in compose
    ...
    assert "ghcr.io/lpalbou/abstractgateway:${ABSTRACTGATEWAY_NVIDIA_IMAGE_TAG:-0.5.0-gpu}" in nvidia_compose
