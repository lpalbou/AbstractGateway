...
- Behavior changes: `POST /api/gateway/runs/{run_id}/images/generate` now generates images through the Runtime/Core `output.modality=image` / `task=image_generation` contract, stores the image as a run artifact, and originally used an image-specific media event. As of 2026-06-02, direct image generation and image edit use the shared child-run progress event `abstract.progress`, matching video/media progress discovery for thin clients. Discovery reports `generated_image.workflow` separately from `generated_image.direct_endpoint`, with the direct route path, event name, supported formats, and max image bytes.
