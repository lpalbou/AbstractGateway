...
- Skills inventory: `GET /api/gateway/skills` — the abstractskill shelf with
  ...
  are labeled `warnings`, never a fabricated list. The shelf is, in order:
  the saved setting `skills.shelf`, then the legacy launch environment value
  (reported as such), then the gateway's own copy in
  `<data dir>/skills/registry`, then a framework checkout's shelf (only when
  that copy is missing), else none; the response names which as
  `shelf_source` (`stored`, `env`, `seeded`, `checkout`, `none`). See
